...
__version__       = "0.5.0"  # TODO: 現在のアプリケーションのバージョンに合わせてください
...
VELA_INVALID_HTML="<title>invalid</title><h1>Invalid Internal Page.</h1><p>Requested resource does not exist.</p>"
...
class BrowserWindow(QMainWindow):
    ...
    def __init__(self, profile):
        ...
        if not self.is_private and session_restore_enabled:
            ...
            if urls:
                for url in urls:
                    if not url.startswith("vela://"):
                        self.add_new_tab(QUrl(url), "読み込み中...")
                        print('[Session Restore]',url)
                    else:
                        if url == "vela://home" or "vela://home?color=black" or url == "vela://bench":
                            self.add_new_tab(QUrl(url), "読み込み中...")
                        else:
                            self.add_new_tab(QUrl(self.default_new_tab_url), "読み込み中...")
                            print('[Session Restore] ReConstructed')
